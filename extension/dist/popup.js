(() => {
  ...
  var PDF_VIEWER_HOST = "mhjfbmdgcfjbbpaeojofohoefgiehjai";
  function sanitizeBaseName(value) {
    const cleaned = String(value ?? "").trim().replace(/[\u0000-\u001f\u007f]+/g, "").replace(/[\\/:*?"<>|]+/g, "_").replace(/\.+$/g, "").trim();
    return cleaned || "CropPDF";
  }
  function deriveBaseNameFromPdfUrl(pdfUrl) {
    if (!pdfUrl) {
      return "";
    }
    try {
      ...
      const lastSegment = String(pdfUrl).split("?")[0].split("#")[0].split("/").filter(Boolean).pop();
      return sanitizeBaseName(String(lastSegment || "").replace(/\.pdf$/i, ""));
    }
  }
  function deriveBaseNameFromTitle(title) {
  ...
  function extractPdfUrlFromTabUrl(tabUrl) {
    ...
    try {
      ...
      if (url.protocol === "chrome-extension:" && url.hostname === PDF_VIEWER_HOST) {
        const fromSearch = url.searchParams.get("file") || url.searchParams.get("src") || url.searchParams.get("url");
        if (fromSearch) {
        ...
        const fromHash = hashParams.get("file") || hashParams.get("src") || hashParams.get("url");
        if (fromHash) {
  ...
  function isLikelyPdfTab(tabUrl) {
    if (!tabUrl) {
      return false;
    }
    try {
  ...
  // extension/src/generated/source-templates.js
  var SOURCE_TEMPLATES = {
    ...
    "FRMW2026": {
      ...
      "authors": "Medway",
      "year": "2026",
      "documentNameTemplate": "Ficha Resumo do Extensivo R3 Cl\xEDnica M\xE9dica: {Nome do Documento}",
      "titleTemplate": "Ficha Resumo do Extensivo R3 Cl\xEDnica M\xE9dica: {Nome do Documento}",
  ...
  // extension/src/source-templates.js
  var SOURCE_TYPE_OPTIONS = Object.entries(SOURCE_TEMPLATES).map(([value, template]) => ({
    value,
    label: template.label || value
  }));
  function getSourceTemplate(sourceType) {
  ...
  function buildSourceTemplateContext({ sourceType, sourceName, imageIdentifier }) {
  ...
  function renderSourceTemplate(template, context) {
    const replacements = [
  ...
  var DEFAULT_DPI = 150;
  var SOURCE_TYPES = SOURCE_TYPE_OPTIONS.concat({ value: "Outros", label: "Outros" });
  var elements = {
  ...
  var currentPdfBaseName = "";
  var lastSettingsPdfBaseName = "";
  var instructionPromptText = "";
  var contentPromptText = "";
  var promptData = null;
  var imageZip = null;
  ...
  function bindEvents() {
    ...
    chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
      ...
      if (tabId === currentTabId || typeof changeInfo.url === "string" || typeof changeInfo.title === "string" || changeInfo.status === "complete") {
        void refreshActivePdf();
      }
    });
  ...
  function fillSourceTypes() {
    elements.sourceType.innerHTML = SOURCE_TYPES.map(({ value, label }) => `<option value="${escapeHtml(value)}">${escapeHtml(label)}</option>`).join("");
  }
  async function refreshActivePdf() {
    ...
    try {
      ...
      const shouldRefreshDerivedFields = lastSettingsPdfBaseName !== fileName || currentPdfBaseName !== fileName;
  ...
  function normalizeSavedSourceType(sourceType, documentName, imageIdentifier, pdfBaseName) {
    if (sourceType === "Livro" && isHarrison22ndReference(`${documentName} ${imageIdentifier} ${pdfBaseName}`)) {
      return "Harrison22nd";
    }
    return sourceType;
  }
  function isHarrison22ndReference(value) {
    return /harrisons?-principles-of-internal-medicine-22nd-edition/i.test(String(value)) || /harrisons?\s+principles\s+of\s+internal\s+medicine\s+22nd\s+edition/i.test(String(value)) || /harrison's\s+principles\s+of\s+internal\s+medicine\s+\(22nd\s+edition\)/i.test(String(value));
  }
  function humanizeBaseName(fileName) {
    return String(fileName).replace(/\.pdf$/i, "").replace(/^Med_[^_]+_/i, "").replace(/-/g, " ").trim();
  }
  ...
  function slugify(value) {
    return String(value).normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^A-Za-z0-9]+/g, "-").replace(/^-+|-+$/g, "").replace(/-{2,}/g, "-") || "Fonte";
  }
  ...
  function buildPromptsFromCurrentFields(data) {
    ...
    const pageBlocks = (data.pages ?? []).map((page) => {
  ...
  function buildInstructionPrompt({ sourceHtmlTemplate, imageBaseName }) {
    return `
Voc\xEA \xE9 um professor experiente de medicina criando flashcards BASIC para provas de resid\xEAncia m\xE9dica/R3.

Quando eu enviar o material, gere poucos flashcards em CSV, selecionando apenas informa\xE7\xF5es high-yield que merecem memoriza\xE7\xE3o ativa. Adote estilo did\xE1tico, formal e explicativo, com perguntas claras e conceituais. Priorize defini\xE7\xF5es, mecanismos, fatores associados, consequ\xEAncias fisiol\xF3gicas, distin\xE7\xF5es entre conceitos pr\xF3ximos, crit\xE9rios, exce\xE7\xF5es, indica\xE7\xF5es, contraindica\xE7\xF5es, sequ\xEAncias e detalhes cl\xE1ssicos de prova.

Cada card deve cobrar uma \xFAnica pergunta. A pergunta deve ser autocontida, precisa e formulada de modo que o aluno entenda exatamente qual conceito est\xE1 sendo testado. Evite perguntas vagas, duplas, excessivamente amplas ou dependentes de contexto oculto.

As respostas devem ser diretas, completas e em linguagem m\xE9dica precisa. Prefira respostas de uma linha quando a resposta n\xE3o envolver lista; use lista apenas quando houver v\xE1rios itens indispens\xE1veis. N\xE3o use respostas telegr\xE1ficas demais, abrevia\xE7\xF5es simb\xF3licas ou frases que pare\xE7am apenas palavras soltas.

As explica\xE7\xF5es devem desenvolver de maneira substancial o racioc\xEDnio subjacente, conectando o conceito \xE0 fisiopatologia, interpreta\xE7\xE3o cl\xEDnica ou forma como costuma ser cobrado em prova. O texto deve soar como um professor explicando um ponto relevante: tecnicamente rigoroso, pedag\xF3gico, seguro e relativamente detalhado, sem se limitar a repetir a resposta m\xEDnima.

Quando pertinente, destaque erros cl\xE1ssicos, invers\xF5es conceituais e associa\xE7\xF5es de prova. Use encadeamento causal expl\xEDcito e contextualiza\xE7\xE3o suficiente para evitar memoriza\xE7\xE3o mec\xE2nica.

Evite:
- inventar dados ausentes;
- misturar v\xE1rios conceitos no mesmo card;
- criar cards sobre informa\xE7\xF5es \xF3bvias ou dedut\xEDveis;
- usar racioc\xEDnio cl\xEDnico amplo quando o material s\xF3 sustenta um conceito espec\xEDfico;
- exagerar no uso de negrito.

Formato do CSV:
\`PERGUNTA,DICA,RESPOSTA,EXPLICA\xC7\xC3O,FONTE,SUBDECK\`

Regras dos campos:
- PERGUNTA, DICA, RESPOSTA, EXPLICA\xC7\xC3O e FONTE devem conter HTML.
- PERGUNTA: clara, conceitual, autocontida e testando uma \xFAnica informa\xE7\xE3o, em HTML.
...
- Em listas HTML, cada item deve come\xE7ar com letra mai\xFAscula ap\xF3s \`<li>\`; use ponto e v\xEDrgula ao fim dos itens intermedi\xE1rios e ponto final no \xFAltimo item.
- Use \`<strong>\` raramente e apenas quando o realce for realmente necess\xE1rio; n\xE3o coloque negrito por h\xE1bito.
- EXPLICA\xC7\xC3O: em HTML, preferencialmente \`<p>\`, com explica\xE7\xE3o did\xE1tica, formal e detalhada, contendo no m\xEDnimo 5 frases completas ou cerca de 120 a 180 palavras, sem quebrar a linha do CSV.
- Na EXPLICA\xC7\xC3O, v\xE1 al\xE9m de repetir o material: desenvolva o racioc\xEDnio com encadeamento causal, contextualizando com conhecimento m\xE9dico consolidado, fisiopatologia, implica\xE7\xE3o pr\xE1tica, erro cl\xE1ssico ou motivo de prova. N\xE3o use explica\xE7\xF5es curtas de uma ou duas frases.
- N\xE3o comece a EXPLICA\xC7\xC3O com frases como "No material", "O material diz", "Segundo o texto", "Segundo o material", "Segundo a fonte" ou equivalentes.
- Evite em qualquer campo formula\xE7\xF5es metalingu\xEDsticas como "segundo a fonte", "segundo o material", "de acordo com o texto" ou "conforme a fonte"; escreva a informa\xE7\xE3o diretamente, como conhecimento m\xE9dico.
- A resposta cobrada deve estar presente no material; o conhecimento externo deve apenas explicar e contextualizar.
- FONTE: sempre HTML, gerada pela fun\xE7\xE3o fonte(paginaVisual).
- SUBDECK: sugest\xE3o curta de subdeck, sem HTML, baseada no tema principal do card. Use hierarquia com :: quando \xFAtil, por exemplo: Cl\xEDnica M\xE9dica::Infectologia::Meningites.

Use este template uma \xFAnica vez para gerar a coluna FONTE:
~~~html
${sourceHtmlTemplate}
~~~

Defina mentalmente ou em c\xF3digo:
fonte(paginaVisual) = template acima com [PaginaVisual] substitu\xEDdo por paginaVisual.

Regras da fonte:
- A \xFAnica parte vari\xE1vel da fonte entre flashcards \xE9 [PaginaVisual].
- Use p\xE1gina visual do PDF/material, como 01, 02, 03.
- N\xE3o coloque p\xE1gina no texto bibliogr\xE1fico da \`<div class="source">\`.
- O src da imagem deve ficar exatamente: ${imageBaseName}-[PaginaVisual].jpg.

Sa\xEDda:
- Se tiver ferramenta de arquivos/Python dispon\xEDvel, gere um arquivo chamado flashcards.csv e forne\xE7a o link para download.
- Para criar o arquivo, use uma lista de linhas e uma fun\xE7\xE3o fonte(paginaVisual), em vez de reescrever manualmente o HTML da fonte em cada linha.
- Use o m\xF3dulo csv ou equivalente para escapar aspas corretamente.
- Se n\xE3o puder criar arquivo para download, entregue somente o CSV puro, sem coment\xE1rios.
- Cabe\xE7alho obrigat\xF3rio: \`PERGUNTA,DICA,RESPOSTA,EXPLICA\xC7\xC3O,FONTE,SUBDECK\`
`.trim();
  }
  function buildContentPrompt(pageBlocks) {
    return `
Gere agora o CSV a partir do material abaixo. Use somente informa\xE7\xF5es presentes nas p\xE1ginas fornecidas. Para cada card, escolha a p\xE1gina visual correspondente, gere FONTE com fonte(paginaVisual) e sugira um SUBDECK coerente.
